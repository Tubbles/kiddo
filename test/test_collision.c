...
void test_rect_rect_overlap(void)
{
    ...
    Vector2 push = resolve_rect_rect(
        (Vector2){0, 0}, 0, 10, 10,
        (Vector2){15, 0}, 0, 10, 10);
...
void test_rect_rect_rotated(void)
{
    ...
    Vector2 push = resolve_rect_rect(
        (Vector2){0, 0}, 45, 10, 10,
        (Vector2){18, 0}, 0, 10, 10);
...
{
    Vector2 push = resolve_circle_circle(
        (Vector2){0, 0}, 10,
        (Vector2){15, 0}, 10);
...
void test_rect_circle_overlap(void)
{
    ...
    Vector2 push = resolve_rect_circle(
        (Vector2){0, 0}, 0, 10, 10,
        (Vector2){15, 0}, 10);
...
{
    Vector2 rc = resolve_rect_circle(
        (Vector2){0, 0}, 0, 10, 10,
        (Vector2){15, 0}, 10);
    Vector2 cr = resolve_circle_rect(
        (Vector2){15, 0}, 10,
        (Vector2){0, 0}, 0, 10, 10);
...
{
    CollisionShape sa = {.count = 1, .prims = {{
        .kind = COLLIDER_RECT, .rect = {10, 10}
    }}};
    CollisionShape sb = {.count = 1, .prims = {{
        .kind = COLLIDER_RECT, .rect = {10, 10}
    }}};

    Vector2 push = resolve_composite(&sa, (Vector2){0, 0}, 0,
                                      &sb, (Vector2){15, 0}, 0);
    Vector2 direct = resolve_rect_rect(
        (Vector2){0, 0}, 0, 10, 10,
        (Vector2){15, 0}, 0, 10, 10);
...
{
    CollisionShape sa = {.count = 1, .prims = {{
        .kind = COLLIDER_CIRCLE, .circle = {20}
    }}};
    CollisionShape sb = {.count = 1, .prims = {{
        .kind = COLLIDER_CIRCLE, .circle = {20}
    }}};

    TEST_ASSERT_TRUE(composite_overlap(&sa, (Vector2){0, 0}, 0,
                                        &sb, (Vector2){30, 0}, 0));
    TEST_ASSERT_FALSE(composite_overlap(&sa, (Vector2){0, 0}, 0,
                                         &sb, (Vector2){50, 0}, 0));
}

void test_composite_wall(void)
{
    CollisionShape shape = {.count = 1, .prims = {{
        .kind = COLLIDER_RECT, .rect = {10, 10}
    }}};
...
void test_tri_circle_overlap(void)
{
    ...
    Vector2 push = resolve_tri_circle((Vector2){0, 0}, verts,
                                       (Vector2){15, 0}, 10);
    float mag = sqrtf(push.x * push.x + push.y * push.y);
    TEST_ASSERT_TRUE(mag > 0.1f);
}
...
void test_tri_rect_overlap(void)
{
    ...
    Vector2 push = resolve_tri_rect((Vector2){0, 0}, verts,
                                     (Vector2){15, 0}, 0, 10, 10);
    float mag = sqrtf(push.x * push.x + push.y * push.y);
    TEST_ASSERT_TRUE(mag > 0.1f);
}
